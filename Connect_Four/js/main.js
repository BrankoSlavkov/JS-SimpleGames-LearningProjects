...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function fillCircle(index) {
        if (SQUARES[index + 7].classList.contains("taken")) {
            if (player === 1 && !SQUARES[index].classList.contains("taken")) {
                SQUARES[index].classList += "taken player-one";
                //change the player
                player = 2;
            } else if (player === 2 && !SQUARES[index].classList.contains("taken")) {
                SQUARES[index].classList += "taken player-two";
                player = 1;
            }
    ...
    for (let i = 0; i < SQUARES.length; i++) {
        (function (index) {
            ...
            SQUARES[i].onclick = fillCircle.bind(this, index);
        })(i);
    }
    ...
    function checkBoard() {
        ...
        for (let i = 0; i < winningArrays.length; i++) {
            var square1 = SQUARES[winningArrays[i][0]],
                square2 = SQUARES[winningArrays[i][1]],
                square3 = SQUARES[winningArrays[i][2]],
                square4 = SQUARES[winningArrays[i][3]];
